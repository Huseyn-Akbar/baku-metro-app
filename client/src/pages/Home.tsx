...
import { ArrowDownUp, BusFront, ChevronRight, CircleHelp, Compass, MapPin, Sparkles } from "lucide-react";
    ...
    <main className="app-shell">
      ...
      <div className="container app-content">
        ...
        <section className="results-section" aria-live="polite">
          ...
          <div className="route-list">{results.map(route => <Card key={route.id} className="route-card"><div className="route-card-top"><div className={`route-number ${route.isExpress ? "express" : ""}`}>{route.number}</div><div className="route-title"><div><h4>{route.title}</h4><p>{route.description}</p></div><Badge variant="outline" className="source-badge">{route.source}</Badge></div></div><div className="route-path">{route.stations.map((station, index) => <span key={station} className="path-stop"><i />{station}{index < route.stations.length - 1 && <b />}</span>)}</div></Card>)}</div>
